...
Store组件实现了Store API用于查询对象存储桶中的历史数据。在做大时间跨度的查询时可能需要遍历对象存储中成百上千个数据块，为了减少对对象存储桶发起查询的频率，提升查询速度，store在做查询时会首先获取每个数据块的元数据进行过滤（如：时间范围和标签），并且将查询频率比较高的数据块索引进行缓存。
